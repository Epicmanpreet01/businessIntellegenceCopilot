...
const HomePage = () => {
  ...
  // Local state for UI feedback - User will integrate with API/Mutations
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [hasData, setHasData] = useState(false); // Placeholder for data check

  const handleFileUpload = (e) => {
    e?.preventDefault();
    setIsAnalyzing(true);
    // User will add their upload mutation here
    setTimeout(() => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-10 pb-10 mt-4 animate-in fade-in duration-500">
      <div>
        <h1 className={`text-3xl lg:text-4xl font-bold ${t.text} mb-3 tracking-tight`}>
      ...
      {hasData && !isAnalyzing && (
        <div className={`${t.panelBg} border ${t.border} rounded-3xl p-6 flex flex-col sm:flex-row items-center justify-between shadow-sm animate-in slide-in-from-bottom-4 duration-500`}>
          <div className="mb-4 sm:mb-0 flex items-start gap-4">
            <div className={`p-3 rounded-xl shrink-0 ${t.primarySoft}`}>
              <Activity className="w-6 h-6" />
            </div>
            <div>
              <h3 className={`text-lg font-bold ${t.text}`}>Active Session Cached</h3>
